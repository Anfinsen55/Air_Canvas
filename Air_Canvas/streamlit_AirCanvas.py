...
while True:
    ...
    if op.multi_hand_landmarks:
        for i in op.multi_hand_landmarks:
            ...
            if x < max_x and y < max_y and x > ml:
                ...
                if (ptime - ctime) > 0.8:
                    curr_tool = getTool(x)
                    st.write("Your current tool set to:", curr_tool)
                    time_init = True
                    rad = 40
            ...
            if curr_tool == "draw":
                ...
                if index_raised(yi, y9):
                    cv2.line(frm, (prevx, prevy), (x, y), (0, 0, 0), thick)
                    prevx, prevy = x, y
                else:
                    prevx = x
                    prevy = y
                ...
                if index_raised(yi, y9):
                    ...
                    if var_inits:
                        cv2.line(frm, (xii, yii), (x, y), (0, 0, 0), thick)
                        var_inits = False
                ...
                if index_raised(yi, y9):
                    ...
                    if var_inits:
                        cv2.circle(frm, (xii, yii), int(((xii - x) ** 2 + (yii - y) ** 2) ** 0.5), (0, 0, 0), thick)
                        var_inits = False
    ...
    # Display the frame with tools and text using Streamlit
    st.image(frm, channels="BGR")
